...
const BookingHistory = () => {
    ...
    const [errorMessage, seterrorMessage] = useState('');
     useEffect(() => {
         ...
         // if(learnerId){
             setisLoading(true)
             fetch(process.env.API_URL+"booking_learner_status/"+learner_id+"/completed",{
                 method:"POST",
                 headers:{
                ...
                <div key={index}>
                    <div className='flex flex-wrap mt-2 justify-between space-y-3 lg:space-y-0'>
                        ...
                        <div className='w-full lg:w-1/2'>
                            ...
                            <h3>{fname+" "+lname}</h3>
                            <p className='text-xs text-gray-500 rounded-md w-fit'>Email: {email}</p>
                            <p className='text-gray-500 underline text-xs'>
                                <i className="bi bi-telephone-fill"></i> {phone}</p>
                        </div>
                    </div>
